  ...
  mask_padding_with_zero=True,):
    ...
    for (ex_index, example) in enumerate(examples):
      ...
      if isinstance(tokenizer, RobertaTokenizer):
        # blame Huggingface.
        inpts = [i for i in inputs["input_ids"] if i != 1437] # empty spaces huggingface inserts for some reason
        inputs["input_ids"] = inpts + [tokenizer.pad_token_id] * (len(inputs["input_ids"]) - len(inpts))
